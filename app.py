import os, json, sqlite3, smtplib, html, socket
...
def send_order_email(order_id, customer, phone, address, note, items, total):
    ...
    try: port=int(os.getenv('EMAIL_PORT','587').strip())
    except: port=587
    user=os.getenv('EMAIL_USER','').strip(); password=os.getenv('EMAIL_PASSWORD','').replace(' ','').strip(); receiver=os.getenv('SHOP_EMAIL','').strip() or user
    if not user or not password or password.startswith('PASTE_') or not receiver:
        return False, 'Chưa cấu hình Gmail App Password trong file .env.'
    msg=EmailMessage(); msg['Subject']=f'🥑 Bơ Shop - Đơn hàng #{order_id}'; msg['From']=user; msg['To']=receiver
    lines=[f'ĐƠN HÀNG #{order_id}','',f'Khách hàng: {customer}',f'Số điện thoại: {phone}',f'Địa chỉ: {address}',f'Ghi chú: {note or "Không có"}','', 'Sản phẩm:']
    for p,q,sub in items: lines.append(f'- {p[1]} x{q}: {sub:,}đ')
    lines += ['',f'TỔNG: {total:,}đ']
    msg.set_content('\n'.join(lines))
    try:
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if path=='/checkout':
            ...
            print(f'Order #{oid}: email={"OK" if ok else "FAILED"}'+(f' - {err}' if err else ''))
            self.redirect(f'/confirmation/{oid}',{}) ; return
        self.send_error(404)
